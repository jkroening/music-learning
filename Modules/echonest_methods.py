from helpers import callAPI, flattenDictCustom
import spotify_methods as sp
from fuzzywuzzy import fuzz

def pullEchoNestSong(api_key, track, album = None, local_link = None):
    ...
    if 'spotify' in track:
        ...
        url_suffix = "?api_key=%s&id=%s&bucket=audio_summary&bucket=id:spotify&format=json" % (api_key, track)
...
def searchEchoNestSong(api_key, link):
    artist, title, album = sp.formatLocalTrack(link)
    url = "http://developer.echonest.com/api/v4/song/search"
