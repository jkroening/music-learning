...
d.pre <- read.csv(args[1], stringsAsFactors = FALSE, encoding = "windows-1252",
                  sep = "\t")
if (length(args) == 2) {
    d.add <- read.csv(args[2], stringsAsFactors = FALSE, encoding = "iso-8859-1")
    out <- system("file --mime new_albums.csv", intern = TRUE)
    if (!grepl("iso-8859", out))
...
## "static" vars
LINK.PRE <- "<a href='"
LINK.MID <- "' class='circle-thumbnail'><img src='http://afireintheattic.com/wp-content/uploads/"
LINK.END <- ".png' alt='' width='50' height='50' class='center' /></a>"
...
cut1 <- cut(d1$POWER.INDEX,
            quantile(d1$POWER.INDEX, probs = seq(0, 1, 0.1)),
            labels = FALSE, include.lowest = TRUE)
cut2 <- cut(d1$POWER.INDEX,
            quantile(d2, probs = seq(0, 1, 0.1)),
            labels = FALSE, include.lowest = TRUE)
...
manualURI <- function(artist, release) {
    ...
    cat("(Enter song title if not available on Spotify): ")
